...
switch ($_POST['req']) {
  ...
  case "list":
    $polls = $pollDB->getAll(); ?>
    <h1>MANAGE POLLS</h1>
    <input type="button" value="Add Poll -x" onclick="polljs.addEdit()"/>
    <?php if (is_array($polls)) {
      foreach ($polls as $p) {
        printf("<div>%s <input type='button' value='Delete' onclick='polljs.del(%u)'/> <input type='button' value='Edit -x' onclick='polljs.addEdit(%u)'/></div>", $p['poll_question'], $p['poll_id'], $p['poll_id']);
      }
    } else {
      echo "No polls found";
    }
  ...
  case "addEdit":
    ...
    <input type="button" value="Save -x" onclick="polljs.save()"/>
    <?php break;
